...
export default function JobDetail() {
  ...
  useEffect(() => {
    if (dataReady && job && !matchDone.current && !matchLoading) {
      runMatch()
    }
  }, [dataReady, job])
...
Write a clean, professional analysis with exactly these four bold headings on their own lines, each followed by one paragraph. Use this exact format:
...
Be direct, warm, and specific. Reference actual details from their resume and About Me. Do not use asterisks inside paragraphs. Do not ask them to upload anything or fill anything out.`
  ...
  async function runTailor() {
    ...
    const resumeText = br
      ? `Resume: "${br.name}"\n\n${br.resume_text.substring(0, 1400)}`
      : 'No resume on file.'
    const prompt = `You are a resume expert. Give exactly 3 tailored resume edit suggestions for this candidate applying to ${job.title} at ${job.company}.
...
[
  {"priority":"HIGH","section":"section name","original":"current text or gap","tailored":"improved version","why":"specific reason this helps","talkAboutIt":"one sentence interview tie-back"},
  {"priority":"MEDIUM","section":"section name","original":"current text or gap","tailored":"improved version","why":"specific reason this helps","talkAboutIt":"one sentence interview tie-back"},
  {"priority":"LOW","section":"section name","original":"current text or gap","tailored":"improved version","why":"specific reason this helps","talkAboutIt":"one sentence interview tie-back"}
]`
    try {
      ...
      if (startIdx !== -1 && endIdx !== -1) {
        setTailorAI(JSON.parse(clean.substring(startIdx, endIdx + 1)))
      } else {
        setTailorAI([])
      }
    } catch { setTailorAI([]) }
...
{"tips":["tip 1","tip 2","tip 3","tip 4"],"whyYou":"2-3 sentences why this specific person is a strong fit referencing their actual background","email":{"subject":"email subject line","body":"full email body with [brackets] for personalization spots"}}`
    try {
      const raw = await callAI(prompt)
      const clean = raw.replace(/```json|```/g, '').trim()
  ...
  async function getPrepFeedback(idx: number, question: string) {
    ...
    const text = await callAI(`Give 2-3 sentence interview answer feedback. Be direct and specific — what's strong, what to sharpen. Question: "${question}" Answer: "${answer}"`)
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: '#f4f2ed', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'sans-serif', color: '#b8a99a' }}>Loading…</div>
  )
  ...
  const prepQuestions = [
    ...
    { q: `Walk me through a project where you communicated complex information clearly.`, snippet: null },
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f4f2ed', fontFamily: 'sans-serif' }}
      ...
      <nav style={{ height: 60, background: '#fff', borderBottom: '1px solid rgba(0,0,0,.07)', display: 'flex', alignItems: 'center', padding: '0 20px', gap: 14, flexShrink: 0 }}>
        <button onClick={() => router.push('/')}
          style={{ background: 'none', border: 'none', color: '#7a7a85', cursor: 'pointer', fontSize: 13, fontFamily: 'sans-serif', display: 'flex', alignItems: 'center', gap: 4 }}>
          ← Back
        </button>
        <div style={{ width: 1, height: 32, background: 'rgba(0,0,0,.1)' }} />
        <div style={{ lineHeight: 1 }}>
          <div style={{ fontFamily: 'Georgia, serif', fontSize: 22, color: '#1a1a1f', letterSpacing: '-.02em' }}>
        ...
        <div style={{ position: 'relative' }} onClick={e => e.stopPropagation()}>
          ...
          {showMenu && (
            <div style={{ position: 'absolute', right: 0, top: '110%', background: '#fff', border: '1px solid rgba(0,0,0,.1)', borderRadius: 10, boxShadow: '0 4px 20px rgba(0,0,0,.12)', minWidth: 200, zIndex: 200, overflow: 'hidden' }}>
              ...
              {isPro ? (
                <button style={{ width: '100%', padding: '10px 14px', textAlign: 'left', background: 'none', border: 'none', fontSize: 13, color: '#3d3d45', cursor: 'pointer', fontFamily: 'sans-serif' }}>
                  Manage subscription
                </button>
              ) : (
                <button onClick={() => { setShowMenu(false); router.push('/') }}
                  style={{ width: '100%', padding: '10px 14px', textAlign: 'left', background: 'none', border: 'none', fontSize: 13, color: '#b8750a', cursor: 'pointer', fontFamily: 'sans-serif' }}>
                  ✦ Upgrade to Pro
                </button>
              )}
              <button onClick={signOut}
                style={{ width: '100%', padding: '10px 14px', textAlign: 'left', background: 'none', border: 'none', borderTop: '1px solid rgba(0,0,0,.07)', fontSize: 13, color: '#7a7a85', cursor: 'pointer', fontFamily: 'sans-serif' }}>
                Sign out
              </button>
            </div>
          )}
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid rgba(0,0,0,.07)' }}>
        <div style={{ padding: '16px 20px 12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
            <div style={{ width: 52, height: 52, borderRadius: 12, background: job.logoBg, color: job.logoColor, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Georgia,serif', fontSize: 18, border: '1px solid rgba(0,0,0,.07)', flexShrink: 0 }}>
              {job.logo}
            </div>
            <div>
              <h1 style={{ fontFamily: 'Georgia,serif', fontSize: 22, color: '#1a1a1f', margin: '0 0 3px', fontWeight: 400 }}>{job.title}</h1>
              <p style={{ fontSize: 13, color: '#7a7a85', margin: 0 }}>
      ...
      <div style={{ display: 'flex', height: 'calc(100vh - 60px - 148px)', overflow: 'hidden' }}>
          ...
          {tab === 'match' && (
            <div>
              ...
              <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.07)', borderLeft: '3px solid #2d5be3', borderRadius: '0 10px 10px 0', padding: '18px 20px', marginBottom: 16 }}>
                <div style={{ fontSize: 10, fontWeight: 600, color: '#b0b0b8', letterSpacing: '.1em', textTransform: 'uppercase' as const, marginBottom: 14 }}>✦ fitted. coach analysis</div>
                {matchLoading
              ...
              <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.07)', borderRadius: 10, padding: 16, marginBottom: 16 }}>
                ...
                {[
                  ...
                  { label: 'Experience',      pct: (job as any).experienceMatch || Math.max(55, score - 8), color: '#1a7a4a' },
                  { label: 'Keywords found',  pct: Math.min(99, score + 1),                              color: '#b8750a' },
                  { label: 'Culture signals', pct: (job as any).cultureMatch   || Math.max(60, score - 4), color: '#6d28d9' },
                ].map(bar => (
              ...
              <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.07)', borderRadius: 10, padding: 16, marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {(job as any).keywords?.length > 0
                    ? (job as any).keywords.map((kw: any) => (
                      ...
                      {tailorAI.map((item: any, i: number) => {
                        ...
                        return (
                          <div key={i} style={{ border: '1px solid rgba(0,0,0,.07)', borderRadius: 10, padding: '14px 16px', marginBottom: 10 }}>
                            ...
                            {item.talkAboutIt && (
                              <div style={{ marginTop: 4, fontSize: 11.5, color: '#7a7a85', lineHeight: 1.5 }}><strong style={{ color: '#3d3d45' }}>Talk about it:</strong> {item.talkAboutIt}</div>
                            )}
                          </div>
                        )
                      })}
                  ...
                  : tailorDone.current
                    ? (
                      <div style={{ textAlign: 'center', padding: '48px 0', color: '#7a7a85' }}>
                        <p style={{ marginBottom: 12 }}>No active resume found. Toggle a resume active from your dashboard to get personalized suggestions.</p>
                        <button onClick={() => router.push('/')} style={{ background: '#2d5be3', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 18px', cursor: 'pointer', fontFamily: 'sans-serif' }}>Go to dashboard</button>
                      </div>
                    )
                    : null
              }
              ...
              {standoutLoading
                ...
                : standoutAI
                  ...
                  : standoutDone.current
                    ? <p style={{ color: '#7a7a85', textAlign: 'center', padding: '48px 0' }}>Could not generate suggestions. Try refreshing.</p>
                    : null
              }
            </div>
          )}
            ...
            <div>
              ...
              <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.07)', borderRadius: 10, padding: 16 }}>
                ...
                <div contentEditable suppressContentEditableWarning
                  style={{ fontSize: 13.5, fontWeight: 600, color: '#1a1a1f', marginBottom: 14, paddingBottom: 14, borderBottom: '1px solid rgba(0,0,0,.07)', outline: 'none' }}>
                  {EMAIL_TEMPLATES[emailType]?.subject}
                </div>
                <div contentEditable suppressContentEditableWarning
                  style={{ fontSize: 13, color: '#3d3d45', lineHeight: 1.8, whiteSpace: 'pre-wrap', outline: 'none', minHeight: 80 }}>
              ...
              {!isPro ? (
                <div style={{ marginTop: 16, background: '#fdf3e3', border: '1px solid rgba(184,117,10,.2)', borderRadius: 8, padding: '12px 14px', fontSize: 12.5, color: '#854f0b', lineHeight: 1.6 }}>
                  ✦ Pro members get personalized coaching tips for each email — timing, subject line strategy, and what to reference.
                </div>
              ) : (
            ...
            <div>
              ...
              {prepQuestions.map((q, i) => (
                <div key={i} style={{ background: '#fff', border: '1px solid rgba(0,0,0,.07)', borderRadius: 10, padding: 16, marginBottom: 10 }}>
                  ...
                  {!isPro && q.snippet && <div style={{ background: '#fdf3e3', borderRadius: 6, padding: '6px 10px', marginBottom: 10, fontSize: 11, color: '#b8750a' }}>✦ Pro — see why they'll ask this from the JD</div>}
                  ...
                  <div style={{ position: 'relative', minHeight: 180, borderRadius: 10, overflow: 'hidden' }}>
                    <div style={{ filter: 'blur(4px)', opacity: .4, pointerEvents: 'none', padding: 16 }}>
                      <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.07)', borderRadius: 10, padding: '14px 16px' }}>
                        ...
                        <div style={{ background: '#eaeffe', borderRadius: 6, padding: 8, fontSize: 12, fontStyle: 'italic' }}>Don't name a number first. Ask for the range…</div>
                      </div>
                    </div>
          ...
          {tab === 'rejected' && (
            !isPro ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 240, gap: 10, textAlign: 'center' }}>
                ...
                <p style={{ fontSize: 13, color: '#7a7a85', maxWidth: 280, lineHeight: 1.6 }}>Get analysis of likely rejection reasons and an email to request real feedback from the hiring team.</p>
                <button onClick={() => router.push('/')} style={{ background: '#b8750a', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'sans-serif' }}>Unlock with Pro</button>
              </div>
            ) : (
              <div>
                ...
                <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.07)', borderRadius: 10, padding: '14px 16px', marginBottom: 12 }}>
                  ...
                  <p style={{ fontSize: 13, color: '#3d3d45', lineHeight: 1.6, margin: 0 }}>
                    Your resume match was {score}% — solid, but often just below the threshold for competitive roles. The most common gaps: missing specific tools ({(job as any).skills?.filter((s: any) => !s.found).map((s: any) => s.name).slice(0, 2).join(' or ') || 'key requirements'}) or not enough role-specific language in your bullets.
                  </p>
                </div>
        ...
        <div style={{ width: 240, flexShrink: 0, background: '#fff', borderLeft: '1px solid rgba(0,0,0,.07)', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ background: '#f4f2ed', borderRadius: 8, padding: '8px 10px', fontSize: 12, color: '#7a7a85' }}>Hi! Quick questions about this role:</div>
