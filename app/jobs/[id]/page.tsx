...
function renderAI(text: string) {
  ...
  return lines.map((line, i) => {
    ...
    const isHeading = /^<strong>[^<]+<\/strong>\s*$/.test(clean.trim()) || line.match(/^\*\*[^*]+\*\*\s*$/)
    if (isHeading) return (
      <div key={i} dangerouslySetInnerHTML={{ __html: clean }}
        style={{ fontSize:13, fontWeight:600, color:'#1a1a1f', letterSpacing:'.01em', marginTop: i===0?0:18, marginBottom:5 }} />
    )
    if (!line.trim()) return <div key={i} style={{ height:4 }} />
    return (
      <div key={i} dangerouslySetInnerHTML={{ __html: clean }}
        style={{ fontSize:13.5, color:'#3d3d45', lineHeight:1.75, marginBottom:4 }} />
    )
  })
}
...
export default function JobDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setLoading(false)
      setDataReady(true)
    }).catch(() => { setLoading(false); setDataReady(true) })
  }, [jobId])

  useEffect(() => {
    if (dataReady && job && !matchDone && !matchLoading) generateMatch()
  }, [dataReady, job])
...
Write a clean, professional analysis with exactly these four bold headings on their own lines, each followed by a paragraph:
...
Be direct, warm, and specific. Name actual details from their resume and About Me. Do not use asterisks inside paragraphs. Do not ask them to fill anything out.`
  ...
  async function generateTailor() {
    ...
    const br = bestResume()
    const resumeSnippet = br ? `"${br.name}"\n${br.resume_text.substring(0, 1400)}` : 'No resume uploaded.'
    const prompt = `You are a resume expert. Give exactly 3 edit suggestions for this candidate applying to ${job.title} at ${job.company}.
Job description: ${job.description.substring(0, 700)}
Resume: ${resumeSnippet}
Return ONLY a JSON array, no markdown:
[{"priority":"HIGH"|"MEDIUM"|"LOW","section":"section name","original":"current text","tailored":"improved text","why":"reason","talkAboutIt":"interview tie-back"}]`
    try {
      const text = await callAI(prompt)
      setTailorAI(JSON.parse(text.replace(/```json|```/g,'').trim()))
    } catch { setTailorAI([]) }
    setTailorDone(true)
  ...
  async function generateStandout() {
    ...
    const br = bestResume()
    const prompt = `Career coach for ${job.title} at ${job.company}.
Job: ${job.description.substring(0, 500)}
About them: ${profile?.about_me?.trim() || 'not provided'}
Resume: ${br ? br.resume_text.substring(0, 600) : 'none'}
Return ONLY JSON, no markdown:
{"tips":["tip1","tip2","tip3","tip4"],"whyYou":"2-3 sentences why this person fits","email":{"subject":"subject","body":"full email body"}}`
    try {
      const text = await callAI(prompt)
      setStandoutAI(JSON.parse(text.replace(/```json|```/g,'').trim()))
  ...
  async function getPrepFeedback(idx: number, question: string) {
    ...
    const text = await callAI(`Give 2-3 sentence interview feedback. Direct and specific. Question: "${question}" Answer: "${answer}"`)
  ...
  if (loading) return <div style={{ minHeight:'100vh', background:'#f4f2ed', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'sans-serif', color:'#b8a99a' }}>Loading…</div>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f4f2ed', fontFamily:'sans-serif' }}>
      ...
      <nav style={{ background:'#2f3e5c', height:54, display:'flex', alignItems:'center', padding:'0 18px', gap:12 }}>
        ...
        {job.url && <a href={job.url} target="_blank" rel="noopener noreferrer" style={{ background:'#2d5be3', color:'#fff', padding:'7px 16px', borderRadius:8, fontSize:13, fontWeight:600, textDecoration:'none' }}>Apply Now →</a>}
      </nav>
      ...
      <div style={{ display:'flex', height:'calc(100vh - 54px - 148px)', overflow:'hidden' }}>
            ...
            <div>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:18 }}>
                {[
                  ...
                  { label:'Personal Fit', value:job.personalFit||Math.max(50,score-5), note:'Based on About Me & profile' },
                  { label:'Likeliness',   value:job.likeliness||Math.max(45,score-8),  note:'Likelihood of progressing' },
                ].map(s => (
              ...
              <div style={{ background:'#fff', border:'1px solid rgba(0,0,0,.07)', borderRadius:10, padding:16, marginBottom:16 }}>
                ...
                <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                  ...
                  {(!job.keywords||job.keywords.length===0) && job.tags.map((t:string) => (
            ...
            <div>
              {tailorLoading ? <Spinner label="Analyzing your resume against this role…" /> :
               tailorDone && tailorAI.length > 0 ? (
                ...
                <p style={{ color:'#7a7a85', textAlign:'center', padding:'48px 0' }}>Upload a resume to get personalized tailor suggestions.</p>
              ) : (
            ...
            <div>
              ...
              {!isPro ? (
                ...
                <div style={{ marginTop:14 }}>
                  ...
                  {emailType==='rejected' && <div style={{ background:'#fff', border:'1px solid rgba(0,0,0,.07)', borderLeft:'3px solid #b8750a', borderRadius:'0 8px 8px 0', padding:'10px 14px', fontSize:12.5, color:'#3d3d45', lineHeight:1.5 }}>Only send if you interviewed at least once. Keep the tone genuinely curious, not disappointed or pushing back.</div>}
                </div>
              )}
            </div>
          )}
                ...
                {isPro ? (
                  <>
                    ...
                    {[
                      ...
                      { stage:'If the range is firm',            script:`"Is there flexibility on start date, title, remote days, or a 90-day review?"`, note:`PTO, title, and performance reviews are negotiable even when base pay isn't.` },
              ...
              <div>
                ...
                <div style={{ background:'#fff', border:'1px solid rgba(0,0,0,.07)', borderRadius:10, padding:'14px 16px', marginBottom:12 }}>
                  ...
                  <p style={{ fontSize:13, color:'#3d3d45', lineHeight:1.6, margin:0 }}>
                    Your resume match was {score}% — solid, but often just below the threshold for competitive roles. The most common gaps: missing specific tools ({job.skills.filter((s:any)=>!s.found).map((s:any)=>s.name).slice(0,2).join(' or ')||'key requirements'}) or not enough role-specific language in your bullets.
                  </p>
                </div>
                <div style={{ background:'#fff', border:'1px solid rgba(0,0,0,.07)', borderRadius:10, padding:'14px 16px', marginBottom:16 }}>
                  ...
                  {['Add any missing tools to your skills section — even "familiar with" counts. ATS systems score on presence, not fluency.','Mirror the exact language from the job description in your first two bullets — that\'s where ATS scoring is heaviest.'].map((tip,i) => (
        ...
        <div style={{ width:240, flexShrink:0, background:'#fff', borderLeft:'1px solid rgba(0,0,0,.07)', overflowY:'auto', display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ flex:1, padding:'14px 16px', display:'flex', flexDirection:'column', gap:6 }}>
            ...
            {!isPro && <div style={{ background:'#fdf3e3', borderRadius:8, padding:'8px 10px', fontSize:11, color:'#7a7a85', marginTop:'auto' }}><span style={{ color:'#b8750a', fontWeight:600 }}>Free — tap a suggestion above</span><br />Upgrade to Pro for unlimited chat</div>}
          </div>
        </div>
      </div>
    </div>
